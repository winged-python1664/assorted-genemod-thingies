...
from scripts.cat.enums import CatThought, CatGroup
...
def grief(dead_cat: "Cat", body: bool):
    ...
    for cat in dead_cat.all_cats.values():
        if cat.dead or cat.status.is_outsider or cat.moons < 1 or cat.status.group_ID != dead_cat.status.get_last_living_group():
            continue
        ...
        if very_high_types:
            ...
            if (
                ...
                or dead_cat.status.get_last_living_group() != CatGroup.PLAYER_CLAN_ID)
            ):
                body_treated = True
                if dead_cat.status.get_last_living_group() == CatGroup.PLAYER_CLAN_ID:
        ...
        if major_chance and not int(random() * major_chance):
            ...
            text = event_text_adjust(cat, text=text, main_cat=dead_cat, random_cat=cat, clan=cat.status.fetch_clan_object(game.clan))
        ...
        if very_low_types:
            ...
            text = event_text_adjust(
                cat, choice(possible_strings), main_cat=dead_cat, random_cat=cat, clan=cat.status.fetch_clan_object(game.clan)
            )
            if cat.ID not in game.clan.grief_strings:
                game.clan.grief_strings[cat.ID] = []
