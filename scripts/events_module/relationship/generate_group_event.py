...
def _find_event_and_cats(
    ...
    outside_cats = [
        ...
        if (c.status.group_ID != main_cat.status.group_ID or c.status.is_outsider) and not c.dead and c.status.is_near()
    ]
    other_clan = (
    ...
    while not chosen_event and possible_events:
        ...
        if not passes_general_constraints(
            event_to_test, involved_cats["m_c"], involved_cats, main_cat.status.fetch_clan_object(game.clan), other_clan
        ):
