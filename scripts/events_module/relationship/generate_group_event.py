...
def _find_event_and_cats(
    ...
    outside_cats = [
        ...
        if (c.status.group_ID != main_cat.status.group_ID or c.status.is_outsider) and not c.dead
    ]
    while not chosen_event and possible_events:
        ...
        temp_involved_cats = find_cats(
            ...
            event=event_to_test,
            other_clan=choice([c for c in game.clan.all_other_clans + [game.clan] if c.group_ID != main_cat.status.group_ID])
