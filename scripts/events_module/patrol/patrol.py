...
class Patrol:
    ...
    def _add_patrol_cats(self, patrol_cats: List[Cat]) -> None:
        ...
        # sets prophet as patrol leader if they're in the patrol
        if CatRank.PROPHET in self.involved_cats.keys():
            possible_leads = self.involved_cats[CatRank.PROPHET]

        # if theres no prophet but there is a med cat, make them pl
        elif CatRank.MEDICINE_CAT in self.involved_cats.keys():
    ...
    def _filter_and_set_patrol(
        ...
        if romantic_patrols:
            ...
            chosen_patrol = self._set_valid_patrol(
                qpr_patrols.copy(), find_qpr=True
            )
    ...
    def _set_valid_patrol(
        ...
        while not chosen_patrol:
            ...
            if not chosen_patrol:
                ...
                Patrol.used_patrols["romance" if find_romance else "qpr" if find_qpr else "normal"].clear()
