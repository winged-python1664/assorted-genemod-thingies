...
Each patch combination is added to a list that dictates the size of the combined patch and how it is inherited. For example, some `little` patches combined may still be categorized as `little` or, if they cover enough of the cat, they could be added as a `mid` patch instead. Use your best judgement in sorting.
